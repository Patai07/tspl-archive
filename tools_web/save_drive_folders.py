...
out_path = "/Users/phu/Desktop/งานพี่กบ/Web/tools_web/drive_folders.txt"
with open(out_path, "w", encoding="utf-8") as f:
    f.write(f"Total folders found: {len(all_names)}\n")
    for cat_name, p_name in sorted(all_names, key=lambda x: (x[0], x[1])):
        f.write(f"Cat: {cat_name} | Folder: '{p_name}'\n")

print(f"Saved {len(all_names)} folders to {out_path}")
