...
echo "╔══════════════════════════════════════════════╗"
echo "║   Batch Archive Pipeline                     ║"
echo "╚══════════════════════════════════════════════╝"
echo ""

echo "▶ Step 1/2 — Sync Drive folders → Batch_Sync sheet"
echo "────────────────────────────────────────────────"
python3 pipeline_batch/batch_sync.py
echo ""

if [ $? -ne 0 ]; then
  echo "❌ batch_sync.py ล้มเหลว กรุณาตรวจ config.json"
...
echo ""
python3 pipeline_batch/batch_classify.py

if [ $? -ne 0 ]; then
  echo "❌ batch_classify.py ล้มเหลว"
  echo "กด Enter เพื่อปิด..."
  read; exit 1
fi
...
echo "╠══════════════════════════════════════════════╣"
echo "║  ขั้นตอนถัดไป:                              ║"
echo "║  1. double-click  batch_review.command       ║"
echo "║     → review รูปที่เป็น PENDING              ║"
echo "║  2. double-click  batch_finalize.command     ║"
