...
for filepath in glob.glob('/Users/phu/Desktop/งานพี่กบ/Web/*.html'):
    ...
    # Match whatever flexible container holds both OCAC and GSR logos
    # It usually starts with flex flex-wrap gap- and containsทั้งสองโลโก้
    pattern = r'<div class="flex flex-wrap gap-.*?">.*?<img src="https://www\.ocac\.go\.th/.*?<img src="assets/กศร\.svg".*?</div>\s*</div>\s*</div>'
    
    new_content = re.sub(pattern, refined_supporters_html, content, flags=re.DOTALL)

    if new_content == content:
        # Second attempt with a slightly wider pattern
        pattern = r'<!-- Supporter 1: OCAC -->.*?<!-- Supporter 2: GSR -->.*?</div>\s+</div>\s+</div>'
        new_content = re.sub(pattern, refined_supporters_html, content, flags=re.DOTALL)
